...
function removeFirstOccurrences(str, value) {
  ...
  const fnStr = str.slice(ind, ind + value.length);
  return str.replace(fnStr, '');
}
...
function removeLastOccurrences(str, value) {
  ...
  if (ind < 0) return str;
  const fnStr = str.slice(ind + value.length);
  return str.slice(0, ind) + fnStr;
}
...
function sumOfCodes(str) {
  if (str) {
    return str.split('').reduce((total, char) => total + char.charCodeAt(0), 0);
  }
...
function orderAlphabetically(str) {
  const sort = str.split('').sort((a, b) => a.charCodeAt(0) - b.charCodeAt(0));
  return sort.join('');
}
...
function countVowels(str) {
  const matched = str.match(/[a, e, i, o, u, y]/gi);
...
function isPalindrome(str) {
  const res = str.toLocaleLowerCase().replace(/\W/g, '');
...
function reverseWords(str) {
  ...
  const map = split.map((el) => el.split('').reverse().join(''));
  return map.join(' ');
}
